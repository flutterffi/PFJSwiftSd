...
final class ObservationLessonServiceTests: XCTestCase {
    func testLoadLessonsReturnsSharedSampleData() throws {
        let service = InMemoryObservationLessonService()
    ...
    func testPersistedServiceLoadsSavedBookmarks() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("pfjswiftsd-observation-service-tests", isDirectory: true)
        ...
        let persistence = ObservationBookmarkPersistence(fileURL: fileURL)
        let lessons = SharedLessonDomain.sampleLessons()
        let targetID = lessons[0].id
        try persistence.save([targetID])

        let service = PersistedObservationLessonService(persistence: persistence)
